...
class Pixel(val name: String, val center: Vector3D, val efficiency: Double = 1.0,
    ...
    fun isHit(track: Track): Boolean {
        ...
        val contains = containsPoint(layer.intersect(track)) ||
                containsPoint(upLayer.intersect(track)) ||
                containsPoint(bottomLayer.intersect(track));
        return contains && (efficiency == 1.0 || rnd.nextDouble() < efficiency);
    }
}